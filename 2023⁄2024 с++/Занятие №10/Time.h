...
//class - более продвинутый сложный тип
class Time{
//к защищённым членам нельзя получить доступ извне
protected:
	unsigned int hours;
	unsigned int minutes;
	unsigned int seconds;
//публичные члены всегда доступны у любой переменной класса
public:
	/*
	 * Конструктор - первое отличие класса, он делит имя класса.
	 * Входные параметры задают способ инициализации переменной.
	 * Параметру может быть присвоено дефолтное значение,
	 * в случае если оно будет не указано при инииализации.
	 */
	Time(const unsigned int &hours, const unsigned int &minutes, const unsigned int &seconds=0):
		//после двоеточия идёт инициализация переменных значениями
		hours{hours}, minutes{minutes}, seconds{seconds}{
		/*
		 * В случае если нужно дополнить значения проверкой или
		 * иными действиями, их забивают в скобки.
		 * assert - останавливает программу если условие не выполнено
		 */
		assert(hours < 24);
		assert(minutes < 60);
		assert(seconds < 60);
		//Мы хотим чтобы пользователь никак не мог нарушить правила
	}
